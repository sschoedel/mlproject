...
def nll(score, labels):
	...
	:rtype: tuple
	"""
	labels01 = np.copy(labels) # create a copy of labels in {0-8}
	labels01[labels01 < 0] = 0
	score[score == 0] = 10**-10
	score[score == 1] = 1-10**-10
	
	objective = np.sum(labels01 * np.log(score) - (1 - labels01) * np.log(1 - score), axis=1)
...
def mlp_objective(model, data, labels, loss_function):
	...
	_, scores, activations, activation_derivatives = mlp_predict(data, model, pre_augmented=True)
...
print(f"training")
best_params = copy.copy(params)
best_params['num_hidden_units'] = [100]
